...
describe('TermClose event', function()
  ...
  --   command('let foo=termopen(["/home/vagrant/neovim/build/bin/shell-test", "EXE"])')
  --   -- command('call jobstop(foo)')
  --   command('call jobstop(b:terminal_job_id)')
  --   eval('g:test_termclose')
  --   retry(nil, nil, function() eq(23, eval('g:test_termclose')) end)
  -- end)

  it('#x triggers when terminal job ends', function()
    command('autocmd TermClose * let g:test_termclose = 23')
    command('terminal')
    command('call jobstop(b:terminal_job_id)')
    eval('1')
